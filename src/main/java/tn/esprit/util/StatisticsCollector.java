...
public class StatisticsCollector {
    ...
    public void recordView(int articleId, String sessionId, Integer userId) {
        String req = "INSERT INTO article_view_event (article_id, session_id, user_id) VALUES (?, ?, ?)";
    ...
    public void recordReaction(int articleId, Integer userId, String reactionType) {
        String req = "INSERT INTO article_reaction_event (article_id, user_id, reaction) VALUES (?, ?, ?)";
        try (PreparedStatement ps = cnx.prepareStatement(req)) {
            ps.setInt(1, articleId);
            if (userId != null) {
                ps.setInt(2, userId);
    ...
    public void recordSearchTerm(String term, int resultCount) {
        String req = "INSERT INTO search_term_log (term, result_count) VALUES (?, ?)";
        try (PreparedStatement ps = cnx.prepareStatement(req)) {
            ps.setString(1, term);
